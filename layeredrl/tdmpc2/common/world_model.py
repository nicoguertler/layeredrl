...
class WorldModel(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # added estimate of termination probability
        self._term_prob = layers.mlp(
            cfg.latent_dim + cfg.action_dim + cfg.task_dim,
            2 * [cfg.mlp_dim],
            1,
        )
    ...
    def term_prob(self, z, a, task):
        """
        Predicts the termination probability.
        """
        if self.cfg.multitask:
            z = self.task_emb(z, task)
        z = torch.cat([z, a], dim=-1)
        return torch.sigmoid(self._term_prob(z))
